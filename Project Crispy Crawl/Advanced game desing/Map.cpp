			...
			for(int c = 0;c <= 24;c++)
			{
				...
				else
				{
					...
					switch (singlevalue)
					{
						...
						case '2':
							//pot;
							maptiles[r][c] = Tile(POT,tls,r,c,rand() % 1 + 0);
							break;
						case '3':
							//damagetile;
							maptiles[r][c] = Tile(FLOOR,tls,r,c,rand() % 3 + 0);
			...
			for(int ii = 0; ii < (int)entitys.size();ii++)//alright lets look through all the entitys on screen.
			{
				if(entitys[i].getUniqueId() != entitys[ii].getUniqueId())//Am i hitting myself?
				{
					...
					if(entitys[i].hasColider(entitys[ii].getMyColider())) //determine if i can colide with it
					{
						...
						if(entitys[i].getBounds().deferedHasColidedWith(entitys[ii].getBounds())) //Check colisions
						{
							//Alright, i have colided with this object, apply damage.
							//TODO: apply actual damage...
							entitys[ii].damageMe(15);
						}
					}
				}
				if(entitys[i].hasColider("PLAYER"))
					{
						if(dung.refToCurrentPlayer()->getBounds().deferedHasColidedWith(entitys[i].getBounds()))
						{
							//ok, this entity has touched the player.
							if(dung.refToCurrentPlayer()->hasImortality())
							{
								//cout << "imo";
							}
							else
							{
								//cout << "Hit";
								dung.refToCurrentPlayer()->damageMe(1);
							}
						}
					}
			}
...
void Map::spawnEnttityInMap(Entity thing)
{
	...
	for(int h = 1;h < SCREEN_TILE_HEIGHT - 1;h++)
	{
		for(int w = 1; w < SCREEN_TILE_WIDTH - 1;w++)
		{
			if(maptiles[h][w].passable())
			{
				vector<float> loc;
				loc.push_back((h*32.0f) + 16.0f);
				loc.push_back((w*32.0f) + 16.0f);
